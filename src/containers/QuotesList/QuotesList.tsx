    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            if (response.data) {
                ...
                const uniqueCategories = Array.from(new Set(quoteFromApi.map(quote => quote.category)));
                    ...
                    {categories.length === 0 ? (
                        ...
                        <List sx={{border: '1px solid', borderColor: '#000'}}>
                            {categories.map((category) => (
                                <ListItem sx={{borderBottom: '1px solid', borderColor: '#c4c2c2'}} button key={category} onClick={() => setSelectedCategory(category)}>
                                    <ListItemText primary={category} sx={{ color: selectedCategory === category ? 'blue' : 'black' }} />
                                </ListItem>
                            ))}
                            <ListItem button onClick={() => setSelectedCategory(null)}>
                                <ListItemText primary="All" sx={{ color: selectedCategory === null ? 'blue' : 'black' }} />
                            </ListItem>
                        </List>
                    )}
                </Grid>
